...
from utils.fast_utils import compute_gaussian_score_fastgs, sampling_cameras, get_edge_mask
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        if hasattr(opt, 'structural_weight') and opt.structural_weight > 0:
                                                      
            edge_mask = get_edge_mask(gt_image).detach()

            adaptive_weight = 0.1 + (opt.structural_weight - 0.1) * edge_mask

            pixel_error = torch.abs(image - gt_image).mean(dim=0) 
            
            weighted_structure_loss = (pixel_error * adaptive_weight).mean()
        ...
        with torch.no_grad():
            view_loss_history[viewpoint_cam.image_name] = loss.item()
        ...
        with torch.no_grad():
            ...
            if iteration < opt.densify_until_iter:
                ...
                current_interval = opt.densification_interval
                
                if hasattr(opt, 'adaptive_densification') and opt.adaptive_densification:
                                                                       
                    phase_progress = (iteration - opt.densify_from_iter) / (opt.densify_until_iter - opt.densify_from_iter)
                    if phase_progress < 0.3:
                        current_interval = 100
                    elif phase_progress < 0.6:
                        current_interval = 250
                    else:
                        current_interval = 500
                                                              
                if iteration > opt.densify_from_iter and iteration % current_interval == 0:
                    size_threshold = 20 if iteration > opt.opacity_reset_interval else None
                    my_viewpoint_stack = scene.getTrainCameras().copy()
                    
                    camlist = sampling_cameras(my_viewpoint_stack, loss_history=view_loss_history)
    ...
    with open(os.path.join(dataset.model_path, "time.txt"), "w") as f:
